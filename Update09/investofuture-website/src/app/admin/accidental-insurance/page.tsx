...
## Admin - Accidental Insurance

You can copy the complete code for the "Accidental Insurance" page from the block below.

**File Path:** `src/app/admin/accidental-insurance/page.tsx`
...
function InsuranceEditor() {
    ...
    const getStorageKey = (userId: string) => `accidental-insurance_${userId}`;
    ...
    useEffect(() => {
        ...
        if (userId) {
          const fetchUser = async () => {
            setIsSearching(true);
            // In a real app, you'd fetch this. Here we use mock data.
    ...
    const loadInsuranceData = (userId: string) => {
        setLoadingData(true);
        try {
            ...
            if (storedData) {
                ...
                // If no data in localStorage, use the initial JSON data as a fallback.
                setInsuranceData(initialInsuranceData);
            }
        } catch (error) {
            console.error("Failed to load insurance data from localStorage", error);
            setInsuranceData(initialInsuranceData); // Fallback to default on error
        }
    ...
    const handleSaveChanges = async () => {
        ...
        setIsSaving(true);
        try {
            localStorage.setItem(getStorageKey(foundUser.id), JSON.stringify(insuranceData));
            toast({
                title: "Changes Saved",
                description: `Accidental insurance data for ${foundUser.personalInfo.fullName} has been saved permanently in this browser.`,
            });
        } catch (error) {
