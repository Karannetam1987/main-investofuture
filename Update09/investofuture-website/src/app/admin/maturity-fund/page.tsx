...
You can copy the complete code for the "Maturity Fund" page from the block below.

**File Path:** `src/app/admin/maturity-fund/page.tsx`
...
function FundEditorInternal() {
    ...
    const getStorageKey = (userId: string) => `maturity-fund_${userId}`;
    ...
    const loadFundData = (userId: string) => {
        setLoadingData(true);
        try {
            ...
            console.error("Failed to load fund data from localStorage", error);
            setFundData(initialMaturityFundData);
        }
    ...
    const handleSaveChanges = async () => {
        ...
        setIsSaving(true);
        try {
            localStorage.setItem(getStorageKey(foundUser.id), JSON.stringify(fundData));
            toast({
                title: "Changes Saved",
                description: `Maturity fund data for ${foundUser.personalInfo.fullName} has been saved permanently in this browser.`,
            });
        } catch (error) {
