...
You can copy the complete code for the "My Documents" page from the block below.

**File Path:** `src/app/admin/my-documents/page.tsx`
...
type Document = { 
    ...
    userId: string;
    date: string;
    fileContent?: string; // Store file as data URL
};
...
export default function MyDocumentsPage() {
    ...
    const getStorageKey = (userId: string) => `user-documents_${userId}`;
    ...
    const handleSearchUser = async () => {
        ...
        try {
            const user = initialUsers.find(u => u.id.toUpperCase() === searchTerm.trim().toUpperCase());
    ...
    const loadDocuments = (userId: string) => {
        setDocsLoading(true);
        try {
            const storedData = localStorage.getItem(getStorageKey(userId));
            if (storedData) {
                setUserDocuments(JSON.parse(storedData));
            } else {
                const docsForUser = initialDocs.filter(d => d.userId === userId);
                const documentsWithUid = docsForUser.map(d => ({ ...d, uid: String(d.id) }));
                setUserDocuments(documentsWithUid);
                // Also save to local storage for future loads
                localStorage.setItem(getStorageKey(userId), JSON.stringify(documentsWithUid));
            }
        } catch (error) {
    ...
    const saveDocuments = async (updatedDocs: Document[]) => {
        ...
        try {
            localStorage.setItem(getStorageKey(foundUser.id), JSON.stringify(updatedDocs));
            setUserDocuments(updatedDocs);
            toast({ title: "Documents Updated", description: "Changes are saved in the browser." });
        } catch (error) {
            toast({ title: "Error Saving", description: "Could not save documents.", variant: "destructive" });
        }
    };
    ...
    const handleFileUpload = async () => {
        ...
        reader.onload = async (e) => {
            ...
            const newDoc: Document = {
                ...
                url: "#", // URL is not used for download, fileContent is
                userId: foundUser.id,
                                    ...
                                    userDocuments.map(doc => (
                                        <TableRow key={doc.uid}>
                                            ...
                                            <TableCell className="text-right space-x-2">
                                                 <a href={doc.fileContent || doc.url} download={doc.name}>
                                                    <Button variant="outline" size="icon" disabled={!doc.fileContent && doc.url === "#"}>
