...
You can copy the complete code for the "Scholarship" page from the block below.

**File Path:** `src/app/admin/scholarship/page.tsx`
...
const SCHOLARSHIP_STORAGE_KEY = "scholarship_data";
...
export default function ScholarshipPage() {
  ...
  useEffect(() => {
      try {
          const storedData = localStorage.getItem(SCHOLARSHIP_STORAGE_KEY);
          if (storedData) {
              setScholarshipData(JSON.parse(storedData));
          } else {
              setScholarshipData(initialScholarshipData);
          }
      } catch (error) {
          console.error("Failed to load scholarship data from localStorage", error);
          setScholarshipData(initialScholarshipData);
      }
      setIsLoading(false);
  }, []);
  ...
  const handleSaveChanges = async () => {
    setIsSaving(true);
    try {
        localStorage.setItem(SCHOLARSHIP_STORAGE_KEY, JSON.stringify(scholarshipData));
        toast({
            title: "Changes Saved",
            description: "Scholarship data has been saved permanently in this browser.",
        });
    } catch (error) {
    ...
    <div className="flex-1 space-y-8 p-4 md:p-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            View, add, or edit scholarship details. This is a global setting and not per-user.
          </CardDescription>
        </CardHeader>
