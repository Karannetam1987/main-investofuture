...
function AccidentalInsuranceContent() {
  ...
  const userId = searchParams.get('userId') || 'INF001'; // Default to INF001 if no userId
  const storageKey = `accidental-insurance_${userId}`;
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (storedData) {
        ...
        // NOTE: This fallback is for when an admin hasn't saved data for this user yet.
        // In a real multi-user app, you might fetch this from a DB.
        // If the default user (INF001) is being viewed, use initial data.
        if (userId === 'INF001') {
          setInsuranceData(initialInsuranceData);
        } else {
          setInsuranceData(null); // No data for other users unless saved by admin
        }
      }
    } catch (error) {
