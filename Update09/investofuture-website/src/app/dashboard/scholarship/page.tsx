...
function ScholarshipContent() {
    ...
    // For a real multi-user app, you'd get the current logged-in user's ID.
    // For this demo, we'll use the one from the query param if it exists (for admin view),
    // otherwise default to a single user's hardcoded data.
    const isAdminView = !!searchParams.get('userId');
    const storageKey = `scholarship_data`; // In this version, scholarship is not user-specific.
    ...
    return (
        ...
        <main className="flex-1 py-12 md:py-16">
            ...
            <div className="mb-6">
                <Link href={isAdminView ? `/admin/user-dashboard?userId=${searchParams.get('userId')}` : "/dashboard"}>
